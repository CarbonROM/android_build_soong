...
func init() {
	pctx.VariableFunc("UseHostBison", func(config interface{}) (string, error) {
		if override := config.(android.Config).Getenv("USE_HOST_BISON"); override != "" {
			return override, nil
		}
		return "no", nil
	})
	...
	if ( "${config.UseHostBison}" == "yes") {
		pctx.SourcePathVariable("yaccCmd", "${config.HostOutExecutable}" + "/bison")
	} else {
